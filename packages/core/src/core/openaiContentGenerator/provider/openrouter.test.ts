...
describe('OpenRouterOpenAICompatibleProvider', () => {
  ...
  describe('buildHeaders', () => {
    ...
    it('should handle unknown CLI version from parent', () => {
      const mockConfigWithUnknownVersion = {
        ...mockCliConfig,
        getCliVersion: vi.fn().mockReturnValue('unknown'),
      };

      const providerWithUnknownVersion = new OpenRouterOpenAICompatibleProvider(
        mockContentGeneratorConfig,
        mockConfigWithUnknownVersion as unknown as Config,
      );

      const headers = providerWithUnknownVersion.buildHeaders();
